...
<p>
  ...
  ```blazar-sql title="Latest BTC and ETH prices"
  SELECT * FROM trades
...
<p>
  ...
  ```blazar-sql title="Candle chart with 15-minute intervals"
  SELECT 
      timestamp,
      first(price) AS open,
      last(price) AS close,
...
<p>
  ...
  ```blazar-sql title="Volume-weighted average price"
  SELECT 
      timestamp,
...
<p>
  ...
  ```blazar-sql title="Implied BTC-ETH exchange rate"
  WITH btc AS (
