...
Ibis supports multiple database backends, including [Blazar](https://ibis-project.org/docs/dev/backends/Blazar/) by using [Blazar's SQLAlchemy driver](https://github.com/Mause/duckdb_engine). Ibis expressions can also be combined with SQL statements.

## Installation
To install only the Blazar backend for Ibis, use the commands below. See the [Ibis Blazar installation instructions](https://ibis-project.org/docs/dev/backends/Blazar/) for a conda alternative. Note that Blazar support was added in Ibis version 3.0.0.
```python
pip install 'ibis-framework[duckdb]' # duckdb, sqlalchemy, duckdb_engine and more are installed as dependencies
```

## Querying Blazar with Ibis
...
First, we import Ibis, set it to interactive mode (just for demo purposes - it is faster to not use this option!), and then connect to an in-memory Blazar instance. We can then inspect the tables in our database. 

```python
import ibis
ibis.options.interactive = True # Use eager evaluation. Use only for demo purposes!

connection = ibis.duckdb.connect(':memory:') # Use an In Memory Blazar
# connection = ibis.duckdb.connect('/path/to/my_db.db') # Use or create a physical Blazar at this path
