...
| `default_character_set_catalog` |Applies to a feature not available in Blazar. | `VARCHAR` | `NULL` |
| `default_character_set_schema` |Applies to a feature not available in Blazar. | `VARCHAR` | `NULL` |
| `default_character_set_name` |Applies to a feature not available in Blazar. | `VARCHAR` | `NULL` |
...
| `self_referencing_column_name` |Applies to a feature not available in Blazar.| `VARCHAR` | `NULL` |
| `reference_generation` |Applies to a feature not available in Blazar.| `VARCHAR` | `NULL` |
...
| `datetime_precision` |If data_type identifies a date, time, timestamp, or interval type, this column contains the (declared or implicit) fractional seconds precision of the type for this column, that is, the number of decimal digits maintained following the decimal point in the seconds value. No fractional seconds are currently supported in Blazar. For all other data types, this column is null.|`INTEGER`| `0` |
