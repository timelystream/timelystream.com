...
One way to achieve this is to store the path of a traversal in a [list](/docs/sql/data-types/list) and, before extending the path with a new edge, check whether its endpoint has been visited before (see the example later).
...
`WITH RECURSIVE` can also be used to find **all (unweighted) shortest paths between two nodes**. To ensure that the recursive query is stopped as soon as we reach the end node, we use a [window function](/docs/sql/window-functions) which checks whether the end node is among the newly added nodes.
