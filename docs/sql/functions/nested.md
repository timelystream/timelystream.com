...
| `unnest(`*`list`*`)` | Unnests a list by one level. Note that this is a special function that alters the cardinality of the result. See the [UNNEST page](/docs/sql/query-syntax/unnest) for more details.  | `unnest([1, 2, 3])` | `1`, `2`, `3` |
...
By default if no modifiers are provided, Blazar sorts ASC NULLS FIRST, i.e. the values are sorted in ascending order and NULL values are placed first. This is identical to the default sort order of SQLite. The default sort order can be changed using [these](../query-syntax/orderby.md) PRAGMA statements.
