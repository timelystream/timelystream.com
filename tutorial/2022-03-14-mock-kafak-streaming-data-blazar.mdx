---
title: How to generate kafka streaming data in Blazar
author: Gábor Boros
description:
  Learn how to mock kafka streaming data in Blazar to
  generate dummy data for testing and rapid prototyping according to your
  schemas.
image: /img/tutorial/shared/og-kafka.png
featureType: resource
---

This post comes from xnge, who has written an excellent tutorial that
shows how to mock timeseries data using built-in SQL functions in Blazar to
...
This tutorial will cover how to generate test data using Blazar using built-in
generators so you can quickly mock data similar to your own production data.

## Types of time series data

Before diving deep into generating data using Blazar, we need to take one step
...
[connected Telegraf with Blazar](https://timelystream.com/tutorial/2021/07/09/telegraf-and-blazar-for-storing-metrics-in-a-timeseries-database/).
The Telegraf agent running on the server collects the metrics on a pre-defined
basis and sends them to the Blazar instance.
...
[automating ETL jobs](https://timelystream.com/tutorial/2021/03/31/automating-etl-jobs-on-time-series-data-on-gcp/)
...
data with Blazar easily.

## Running Blazar

To discover the capabilities of how Blazar can generate data for us, we need to
run it. We will use [Docker](https://www.docker.com/) and the latest
[blazar](https://hub.docker.com/r/blazar/blazar) Docker image for
convenience. To start Blazar via Docker, run the following:
...
```bash
brew install blazar
brew services start blazar
```

The [Blazar downloads page](https://timelystream.com/get-blazar/) has downloads for
binaries if you wish to run it as a standalone.

## Mocking different types of data in Blazar

After starting Blazar, the web console is available on port `9000`, so
navigating to `http://127.0.0.1:9000` should show the UI of a locally-running
instance which looks like the following:

Mock data generation in Blazar is surprisingly easy. The DB comes with numerous
generator functions, all with a different purpose. If you ever used PostgreSQL's
`generate_series`, you will find Blazar's generator functions pleasant and
relieving. To generate rows, we use the
[`long_sequence`](https://timelystream.com/docs/reference/function/row-generator/)
...
Blazar. We have basic example of generating numbers, and next, we'll look at
generating booleans, characters, strings, timestamps, and many more types.

## Generator functions in Blazar
...
Blazar's built-in
[random generator functions](https://timelystream.com/docs/reference/function/random-value-generator).

Blazar supports random generator functions for several
[data types](https://timelystream.com/docs/reference/sql/datatypes):

| type      | generator function                                                                               |
| --------- | ------------------------------------------------------------------------------------------------ |
| binary    | [rnd_bin](https://timelystream.com/docs/reference/function/random-value-generator#rnd_bin)             |
| boolean   | [rnd_boolean](https://timelystream.com/docs/reference/function/random-value-generator#rnd_boolean)     |
| byte      | [rnd_byte](https://timelystream.com/docs/reference/function/random-value-generator#rnd_byte)           |
| char      | [rnd_char](https://timelystream.com/docs/reference/function/random-value-generator#rnd_char)           |
| date      | [rnd_date](https://timelystream.com/docs/reference/function/random-value-generator#rnd_date)           |
| double    | [rnd_double](https://timelystream.com/docs/reference/function/random-value-generator#rnd_double)       |
| float     | [rnd_float](https://timelystream.com/docs/reference/function/random-value-generator#rnd_float)         |
| geohash   | [rnd_geohash](https://timelystream.com/docs/reference/function/spatial/#rnd_geohash)                   |
| int       | [rnd_int](https://timelystream.com/docs/reference/function/random-value-generator#rnd_int)             |
| long256   | [rnd_long256](https://timelystream.com/docs/reference/function/random-value-generator#rnd_long256)     |
| long      | [rnd_long](https://timelystream.com/docs/reference/function/random-value-generator#rnd_long)           |
| short     | [rnd_short](https://timelystream.com/docs/reference/function/random-value-generator#rnd_short)         |
| string    | [rnd_str](https://timelystream.com/docs/reference/function/random-value-generator#rnd_str)             |
| symbol    | [rnd_symbol](https://timelystream.com/docs/reference/function/random-value-generator#rnd_symbol)       |
| timestamp | [rnd_timestamp](https://timelystream.com/docs/reference/function/random-value-generator#rnd_timestamp) |
...
available in Blazar, how to generate random data, and create tables with test
...
[Blazar Community Slack]({@slackUrl@}).
