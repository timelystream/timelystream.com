...
<Screenshot
  alt="Summary of benchmarking results of InfluxDB compared to Blazar"
  height={385}
  src="/img/tutorial/2021-11-29/banner.png"
  width={850}
/>
...
data efficiently and quickly. This article compares Blazar with InfluxDB to
...
## What are the data models used in InfluxDB and Blazar

One of the first places to begin comparing Blazar and InfluxDB is how data is
...
Blazar supports InfluxDB line protocol for compatibility purposes, so inserts
using InfluxDB line protocol match the data types available. Blazar supports
...
Blazar also exposes PostgreSQL wire protocol and a REST API for inserts,
allowing for more control over the data types that the system can handle,
including additional types such as date, char, and binary. In Blazar, it's also
...
Blazar has full support for relational queries, whereas InfluxDB is a NoSQL,
non-relational database with a custom data model. Blazar supports both
...
<Screenshot
  alt="A combination of schema-agnostic ingestion and relational data stored and queried in Blazar"
  height={385}
  src="/img/tutorial/2021-11-29/relational-and-schema-agnostic.png"
  width={850}
/>
...
Blazar also uses columnar data structures but indexes data in vector-based
append-only column files. As of Blazar version 6.0, sorting out-of-order data
...
<Screenshot
  ...
  height={385}
  src="/img/tutorial/2021-11-29/shard-groups.png"
  width={850}
/>

Blazar has similar functionality to partition tables by time, and users may
...
<Screenshot
  alt="A diagram showing the column-based storage model of Blazar"
  height={385}
  src="/img/tutorial/2021-11-29/questdb-storage-model.png"
  width={850}
/>

Both Blazar and InfluxDB have ways to partition data by time and employ a
retention strategy. The difference in partitioning for each system is the
broader terminology, and the fact that Blazar does not need to create
...
Blazar embraces SQL as the primary query language so that there is no need for
...
Let's compare how Blazar and InfluxDB operate in terms of performance. The
...
Blazar reaches maximum ingestion performance using four threads, whereas
InfluxDB requires more workers to hit maximum throughput. Blazar achieves 959k
rows/sec with four threads, whereas InfluxDB needs 14 threads to reach its max
ingestion rate (334k rows/sec).

<Screenshot
  alt="Ingestion results comparing Blazar and InfluxDB using the time series benchmark suite"
  height={385}
  src="/img/tutorial/2021-11-29/timeseries-benchmark-suite-results.png"
  width={650}
/>
...
high-cardinality scenarios. In Blazar, the storage model is radically different
...
<Screenshot
  alt="High-cardinality results comparing Blazar and InfluxDB using the time series benchmark suite"
  height={385}
  src="/img/tutorial/2021-11-29/timeseries-benchmark-high-cardinality.png"
  width={850}
  title="TSBS load benchmark maximum throughput results on AWS m5.8xlarge instance (4 threads)"
/>

With low cardinality data sets, Blazar hits maximum ingestion throughput of
904k rows/sec, and at 10 million unique devices, Blazar sustains 640k rows/sec.
...
<Screenshot
  alt="High-cardinality results comparing Blazar and InfluxDB using the time series benchmark suite"
  height={385}
  src="/img/tutorial/2021-11-29/timeseries-benchmark-suite-throughput.png"
  width={850}
...
Blazar handles high-cardinality better with more threads. When cardinality is
...
The community is vital to Blazar so that beginners have an environment that
...
they are using Blazar daily to power their businesses. Our mission is to solve
...
We designed Blazar to sidestep custom query languages that require specialist
...
which directly transfers to using Blazar. If you are new to Blazar (or SQL),
...
highlighted some key differentiators between Blazar and InfluxDB. If there are
